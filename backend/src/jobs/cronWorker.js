...
const { pool } = require('../config/db'); // 👈 Asegúrate de desestructurar { pool } si se exporta como objeto
...
const initCronJobs = () => {
  ...
  cron.schedule('1 0 * * *', async () => {
    ...
    try {
      // 👈 Usamos pool.query directamente (así no necesitas abrir y liberar clientes manualmente)
      await pool.query(`
        UPDATE Cuenta c 
      ...
      `);
      console.log('[CRON] Barrido diario ejecutado exitosamente.');
    } catch (error) {
    ...
    try {
      ...
      await client.query(`
        ...
        WHERE nro_solicitud IN (
          SELECT nro_solicitud FROM Solicitud_Servicio WHERE fecha_cierre < CURRENT_DATE - INTERVAL '1 year'
        )
      `);
      ...
      await client.query(`
        ...
        WHERE fecha_fin_cobertura < CURRENT_DATE - INTERVAL '1 year' AND beneficios_activos = FALSE
      `);
      
      await client.query('COMMIT');
      console.log('[CRON] Purga anual completada de forma segura.');
    } catch (error) {
  ...
  cron.schedule('50 23 28-31 * *', async () => {
    ...
    try {
      ...
      await client.query(`
        ...
        SELECT fc.nro_solicitud, fc.fecha_apertura, 
               COALESCE((
                 SELECT SUM(lc.cantidad * lc.precio_unitario) 
                 FROM Linea_Cargo lc 
                 WHERE lc.nro_solicitud = fc.nro_solicitud AND lc.fecha_apertura_folio = fc.fecha_apertura
               ), 0) AS saldo, 
               'Pendiente', s.cedula_miembro
        FROM Folio_Consumo fc
        JOIN Solicitud_Servicio s ON fc.nro_solicitud = s.nro_solicitud
        WHERE NOT EXISTS (
          SELECT 1 FROM Factura f WHERE f.nro_solicitud = fc.nro_solicitud AND f.fecha_apertura_folio = fc.fecha_apertura
        )
      `);
