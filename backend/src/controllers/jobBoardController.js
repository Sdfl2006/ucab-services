...
const createVacante = async (req, res, next) => {
    try {
        ...
        const { rows } = await pool.query(query, [
            nombre_entidad, cargo, fecha_oferta, responsabilidades, beneficios, perfil_buscado
        ]);

        res.status(201).json({
            success: true,
            message: 'Vacante laboral publicada exitosamente en la plataforma.',
            data: rows[0]
        });
    } catch (error) {
        if (error.code === '23505') {
...
const closeVacante = async (req, res, next) => {
    try {
        ...
        res.status(200).json({
            success: true,
            message: 'La vacante laboral ha sido cerrada oficialmente.',
            data: rows[0]
        });
    } catch (error) {
        next(error);
    ...
    try {
        ...
        const egresadoRes = await pool.query(
            `SELECT e.*, m.nombres, m.apellidos 
             FROM Egresado e 
             JOIN Miembro_comunidad m ON e.cedula = m.cedula 
             WHERE e.cedula = $1`, 
            [cedula]
        );
        ...
        // 2. Algoritmo SQL inteligente: Busca vacantes disponibles y calcula relevancia
        // Coincidencia por palabras clave entre el título del egresado y el perfil/cargo buscado
...
const applyToVacante = async (req, res, next) => {
    try {
        ...
        res.status(201).json({
            success: true,
            message: 'Postulación registrada exitosamente. Tu perfil está en revisión por el aliado externo.',
            data: rows[0]
        });
    } catch (error) {
        if (error.code === '23505') {
    ...
    try {
        const query = `
            SELECT 
                v.nombre_entidad,
                v.cargo,
                COUNT(p.cedula) AS total_postulantes,
                COUNT(CASE WHEN p.estatus_seleccion = 'contratado' THEN 1 END) AS total_contratados,
                COUNT(CASE WHEN p.estatus_seleccion = 'en revisión' THEN 1 END) AS en_proceso,
                ROUND(
                    (COUNT(CASE WHEN p.estatus_seleccion = 'contratado' THEN 1 END)::numeric / 
                    NULLIF(COUNT(p.cedula), 0)) * 100, 2
                ) AS porcentaje_efectividad
            FROM Vacante v
            LEFT JOIN Postulacion p ON v.nombre_entidad = p.nombre_entidad 
                 AND v.cargo = p.cargo AND v.fecha_oferta = p.fecha_oferta
            GROUP BY v.nombre_entidad, v.cargo
            ORDER BY porcentaje_efectividad DESC NULLS LAST;
        `;
        const { rows } = await pool.query(query);
